...
All the attributes are prefixed with "__moduler_"
"""
...
def todo(todo_info_or_func: str):
    ...
    It sets the attribute __moduler_todo in the __dict__ of the function or class
    """
...
def _todo_decorator(func_or_class, todo_info):
    if isinstance(func_or_class, type):
        func_or_class.__moduler_todo = todo_info
    else:
        func_or_class.__dict__["__moduler_todo"] = todo_info
...
def example(func_or_class):
    func_or_class.__dict__["__moduler_example"] = True
    return func_or_class
